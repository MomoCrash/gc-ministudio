...
class Player(Entity):
    ...
    def DrawArrow(self, window):
        if self.arrow != None:
            Assets.GetSprite(SpritesRef.TOMAHAWK).draw(window, self.arrow.transform.position,
                                                       self.arrow.transform.scale)
...
class Mob(Entity):
    ...
    def tryThrow(self, player: Player):
        ...
        if self.maximum_distance < self.transform.position.distanceTo(
                player.transform.position) < self.maximum_throw_distance:
            self.hammer = GameObject(position=Vector2(self.transform.position.x, self.transform.position.y))
            self.Vecteur_directeur = Vector2(player.transform.position.x - self.transform.position.x,
    ...
    def draw(self, window: pygame.Surface, player: Player):
        ...
        if self.hammer != None:
            Assets.GetSprite(SpritesRef.TOMAHAWK).draw(window, self.hammer.transform.position,
                                                       self.hammer.transform.scale)
